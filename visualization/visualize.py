...
class MetricData:
    ...
    def smooth(self, alpha: float = 0.1) -> 'MetricData':
        ...
        """
        smoothed_data = np.full_like(self.data, np.nan)
        
        # Apply EMA smoothing to each layer and metric, ignoring NaNs
        for l in range(self.n_layers):
            for m in range(self.n_metrics):
                ...
                if idx.size == 0:
                    ...
                    continue
                first = idx[0]
                smoothed[first] = data[first]
                # EMA forward, carrying last smoothed value through NaNs
                for i in range(first + 1, len(data)):
                    if np.isnan(data[i]):
                        smoothed[i] = smoothed[i-1]
                    else:
                        prev = smoothed[i-1] if not np.isnan(smoothed[i-1]) else data[i]
                        smoothed[i] = alpha * data[i] + (1 - alpha) * prev
                smoothed_data[:, l, m] = smoothed
...
class MetricAggregator:
    ...
    def best_by_final_loss(runs: List[RunData], window: int = 10) -> AggregationResult:
        ...
        def score(run: RunData) -> float:
            ...
            losses = run.metrics['losses'].data[:, 0, 0]  # Get 1D array
            tail = losses[-window:]
            # Use NaN-aware mean; if no valid points, return inf
            valid = tail[~np.isnan(tail)]
            return float(np.mean(valid)) if valid.size > 0 else float('inf')
